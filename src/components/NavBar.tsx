  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-black/80 dark:bg-white/80 backdrop-blur-md border-b border-gray-800 dark:border-gray-200">
      <div className="max-w-7xl mx-auto px-6 py-4">
        ...
        {/* Mobile Menu */}
        {isMenuOpen && (
          <div className="md:hidden border-t border-gray-800 dark:border-gray-200 py-4 flex flex-col space-y-4">
            {navlinks.map((link: NavLink, index: number) => {
              const isActive = activeSection === link.href.substring(1);
              return (
                <a
                  ...
                  className={`text-sm font-medium px-2 py-1 transition-colors duration-300 ${
                    isActive
                      ? "text-white dark:text-black"
                      : "text-gray-300 dark:text-gray-600 hover:text-white dark:hover:text-black"
                  }`}
